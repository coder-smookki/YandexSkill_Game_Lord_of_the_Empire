def getConfig(event):
    config = {
        "tts": """Ильин Кирилл. Плюснин Александр. Кунакбаев Радмир. Лесовой Кирилл. Кутников Родион. Чтобы продолжить взаимодействовать с навыком, произнесите определённый вариант ответа: Повторить ещё раз, Что ты умеешь?, Назад, Выход.""",
        "buttons": [
            "Повторить ещё раз",
            "Что ты умеешь?",
            "Назад",
            "Выход"
            ,
        ],
        "card": {
            "type": "BigImage",
            "image_id": "1521359/3349afd66fa90845fd03",
            "title": "СОЗДАТЕЛИ НАВЫКА",
            "description": """
            Ильин Кирилл - Тимлид (руководит процессом разработки),
            Кутников Родион - Дизайнер (рисует арты и придумывает дизайн для навыка)
            Плюснин Александр - Главный Разработчик (отвечает за всю оболочку навыка),
            Кунакбаев Радмир - Разработчик и Сценарист (дополняет функционал и придумывает весь сценарий игры),
            Лесовой Кирилл - Разработчик и Тестировщик (обеспечивает навык полным функционалом игры и фиксирует ошибки).
            Чтобы продолжить взаимодействовать с навыком, произнесите определённый вариант ответа: Повторить ещё раз, Что ты умеешь?, Назад, Выход.
            """
        },
    }

    session_state = {"branch": "whoAreCreators"}

    return {
        'message': config["tts"],
        "tts": config["tts"],
        "buttons": config["buttons"],
        "card": config["card"],
        "session_state": session_state,
    }
