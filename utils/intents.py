...
RepeatIntents = ['что', 'повтори', 'ещё раз', 'еще раз', 'репит', 'повтор', 'не понял', 'не догнал',
                 'не вкурил', 'ало', 'ась', 'але', 'алё']
HelpIntents = ['помог', 'помощь', 'хелп', 'подмога', 'поддержка', 'не умею', 'не знаю', 'как ', 'научи', 'расскаж']
...
HowToUseIntents = ['использовать', 'применять', 'экплуатировать', 'пользоваться', 'употреблять',
                   'распоряжаться', 'юзать', 'использовать', 'польз', 'как играть']
